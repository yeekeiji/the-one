 ...
 */
public class HeraDebugRouter extends ActiveRouter {
    ...
    public static final String HERA_NS = "HeraDebugRouter";
    /**
    ...
    /** Matrix storage file setting keyword
     */
    public static final String DUMPFILE_S = "dumpTo";
    /** nodes to watch setting keyword
     */
    public static final String WATCHNODE_S = "watchNodes";
    ...
    /** filepath string for dumpfile
     */
    private String dumpFile;
    /** array of node strings to watch
     */
    private String[] watchNodes;
     ...
     */
    public HeraDebugRouter(Settings s) {
        super(s);
        Settings heraSettings = new Settings(HERA_NS);
        secondsInTimeUnit = heraSettings.getInt(SECONDS_IN_UNIT_S);
        // dumpFile should be the absolute path to the file you want to dump M
        // to 
        dumpFile = heraSettings.getSetting(DUMPFILE_S);
        watchNodes = heraSettings.getCsvSetting(WATCHNODE_S);

        // checking for HeraDebugRouter.lambda = x, y, z
        ...
        // check for HeraDebugRouter.gamma & HeraDebugRouter.sizeofGamma
        if (heraSettings.contains(HOPS_S) && 
     ...
     */
    protected HeraDebugRouter(HeraDebugRouter r) {
        super(r);
        this.secondsInTimeUnit = r.secondsInTimeUnit;
        this.dumpFile = r.dumpFile;
        this.watchNodes = r.watchNodes;
        this.alpha = r.alpha;
        this.hops = r.hops;
    ...
    public void changedConnection(Connection con) {
        ...
        if (con.isUp()) {
            ...
            for(int i = 0; i < watchNodes.length; ++i){
                if( getHost().toString().equals( watchNodes[i] )){
                    this.dumpToFile();
                }
            }
        }
    ...
    private Tuple<Message, Connection> tryOtherMessages() {
        ...
        for ( Connection con : getConnections() ) {
            ...
            HeraDebugRouter othRouter = ( HeraDebugRouter )other.getRouter();
    ...
    public MessageRouter replicate() {
        HeraDebugRouter r = new HeraDebugRouter(this);
        return r;
    }

// dumping M matrix to file dumpFile
    public void dumpToFile(){
        String EOL = System.getProperty("line.separator");
        try(Writer writer = new FileWriter( dumpFile, true )) {
            String currTime = Double.toString(SimClock.getTime());
            // iterate through host node's reach matrix and output to file
            for (Map.Entry<DTNHost, Map<Integer, Double>> entry : this.reach.entrySet()){

                // writer is appending values to a line in the file 
                writer.append(currTime)
                      .append(',')
                      .append(getHost().toString())
                      .append(',')
                      .append(entry.getKey().toString());

                // Need to iterate through inner map's values. Can't dump map
                // directly in csv form
                for(Map.Entry<Integer, Double> inner : entry.getValue().entrySet()){
                    writer.append(',')
                          .append(inner.getValue().toString());
                }
                // put end of line character to signify end of data for this
                // host
                writer.append(EOL);
            }
            // catch errors
