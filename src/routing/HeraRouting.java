...
/**
 * Implementation of Hope Expansion Routing Algorithm (HERA) as described in
...
public class HeraRouter extends ActiveRouter {
    ...
    // need to change name preds -> ? 
	private Map<Integer, Map<DTNHost, Double>> preds;
	...
	public HeraRouter(Settings s) {
		...
		if (heraSettings.contains(BETA_S)) {
			beta = heraSettings.getDouble(BETA_S);
		}
		else {
			beta = DEFAULT_BETA;
		}
		if (heraSettings.contains(P_INIT_S)) {
			p_init = heraSettings.getDouble(P_INIT_S);
		}
		else{
	...
	private void updateTransitivePreds(DTNHost host) {
		...
		Map<DTNHost, Double> othersPreds =
			((HeraRouter)otherRouter).getDeliveryPreds();
	...
	private class TupleComparator implements Comparator
		...
		public int compare(Tuple<Message, Connection> tuple1,
			...
			double p1 = ((HeraRouter)tuple1.getValue().
					getOtherNode(getHost()).getRouter()).getPredFor(
					tuple1.getKey().getTo());
			// -"- tuple2...
			double p2 = ((HeraRouter)tuple2.getValue().
					getOtherNode(getHost()).getRouter()).getPredFor(
					tuple2.getKey().getTo());
